...
`;
  ...
  if (error) {
    console.warn('Failed to set permissions for migration file:', (error as Error).message);
  }
}
...
export default async function migrationCommand() {
  ...
  if (argv.includes('migration:generate')) {
    ...
    console.log('Valid commands: migration:generate, migration:run, migration:revert');
  }
